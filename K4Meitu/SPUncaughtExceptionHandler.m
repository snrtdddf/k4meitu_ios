...
- (void)alertView:(UIAlertView *)anAlertView clickedButtonAtIndex:(NSInteger)anIndex {
    if (anIndex == 0) {
//        dismissed = YES;
        [anAlertView removeFromSuperview];
    }else if (anIndex==1) {
        //继续
        if (action) {
...
//    _message_exception = [NSString stringWithFormat:NSLocalizedString(@"异常原因如下:\n%@\n%@", nil), [exception reason], [[exception userInfo] objectForKey:UncaughtExceptionHandlerAddressesKey]];
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:titleStr message:_message_alert delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"继续", nil), nil];
