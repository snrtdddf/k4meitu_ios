...
@interface YXViewPagerBaseViewController ()<UIScrollViewDelegate,YXViewPagerEventDelegate,YXViewPagerTopViewDelegate>
...
- (void)initData{
    ...
    for (int i=0; i<dataArray.count; i++) {
        ...
        [_dataArr addObject:viewModel];
    }
}

- (void)didSelegateItemTag:(NSInteger)tag{
    [self tabItemSelected:tag];
}

- (void)initTopView{
    _topView = [[YXViewPagerTopView alloc] initWithFrame:CGRectMake(0,0, self.view.width, 60)];
    ...
    [_topView renderUIWithArray:_dataArr];
...
- (void)initContentScroller{
    ...
    _contentScroller.contentSize = CGSizeMake(_contentScroller.width*_dataArr.count, _contentScroller.height);
...
- (void)loadSubViewController{
    ...
    YXViewPagerItemViewModel *viewModel = _dataArr[_currentIndex];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
   ...
   CGFloat pageRate = scrollView.contentOffset.x/scrollView.width;
    ...
    if (_type == YXViewPagerTopViewTypeForNoScroll) {
        //底部线条时
//        maskView.frame = CGRectMake(rate*self.view.width, itemHeight-2, maskView.width, 1);
        //长方块时
...
//            maskView.frame = CGRectMake(self.view.width/2-itemWidth/2, itemHeight-2, maskView.width, 1);
...
//            maskView.frame = CGRectMake(tabWidth*rate, itemHeight-2, maskView.width, 1);
...
//            maskView.frame = CGRectMake(self.view.width/2+tabWidth*rate-(tabWidth-self.view.width/2), itemHeight-2, maskView.width, 1);
